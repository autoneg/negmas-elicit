...
class BaseVOIElicitor(BaseElicitor):
    ...
    VOI-based elicitation algorithms evaluate the expected value of acquiring
    information about uncertain utility values and compare it against the cost
    of elicitation. They elicit preferences when the expected benefit exceeds
    the cost.

    This approach was introduced to negotiation by Baarslag and Kaisers (2017).

    References
    ----------
    .. [BaarslagKaisers2017] Baarslag, T., & Kaisers, M. (2017).
       The Value of Information in Automated Negotiation: A Decision Model
       for Eliciting User Preferences. In Proceedings of the 16th Conference
       on Autonomous Agents and MultiAgent Systems (AAMAS '17), 391-400.
...
class VOIElicitor(BaseVOIElicitor):
    """
    The Optimal Querying Agent (OQA) for preference elicitation during negotiation.

    This algorithm evaluates the value of information for each possible query
    and selects queries that maximize the expected utility improvement minus
    the elicitation cost. It computes an optimal offering policy and updates
    it as new information is acquired.

    The algorithm was introduced by Baarslag and Kaisers (2017).
...
class VOIFastElicitor(BaseVOIElicitor):
    """
    FastVOI algorithm for efficient utility elicitation during negotiations.

    This algorithm improves upon the standard VOI approach by using incremental
    updates to the optimal policy, avoiding full recomputation when utility
    estimates change. This results in significantly faster elicitation while
    maintaining the same quality of decisions.

    The algorithm was introduced by Mohammad and Nakadai (2018).

    References
    ----------
    .. [MohammadNakadai2018] Mohammad, Y., & Nakadai, S. (2018).
       FastVOI: Efficient utility elicitation during negotiations.
       In International Conference on Principles and Practice of Multi-Agent
       Systems (PRIMA 2018), 560-567. Springer.
...
class VOIOptimalElicitor(BaseElicitor):
    """
    Optimal VOI elicitor with continuous query generation.

    This algorithm extends the VOI approach by automatically generating optimal
    queries of the form "Is u(o) > x?" without requiring the user to specify
    a finite set of queries beforehand. It finds the optimal threshold x that
    maximizes the value of information for each outcome.

    The algorithm was introduced by Mohammad and Nakadai (2019).

    References
    ----------
    .. [MohammadNakadai2019] Mohammad, Y., & Nakadai, S. (2019).
       ...
       Agents and MultiAgent Systems (AAMAS '19), 242-250.
       International Foundation for Autonomous Agents and Multiagent Systems.
